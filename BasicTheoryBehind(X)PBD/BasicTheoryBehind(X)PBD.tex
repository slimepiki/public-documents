...
The theory directly used in XPBD is derived from \pname{Interactive simulation of elastic deformable materials}\cite{Servin2006InteractiveSO} that uses the theory for \gls{LagrangianMechanics}.

Besides that, some papers also influence XPBD, \mainpname{Efficient simulation of inextensible cloth}\cite{EfficientGoldenthai} and \pname{Strain Based Dynamics}\cite{StrainBasedDyn} are examples of them.
...
\section{Physics}
\subsection{Newtonial mechanics}
...
\subsection{What is the local/global solver}
...
\subsection{Explicit/implicit euler scheme}
...
\subsubsection{{\sl A Versatile and Robust Model for Geometrically Complex Deformable Solids}\cite{VersatileTeschner}}
...
\subsubsection{{\sl XPBD : position-based simulation of compliant constrained dynamics}\cite{XPBD}}
...
\subsubsection{{\sl Projective Dynamics: Fusing Constraint Projections for Fast Simulation}\cite{ProjDyn}}
\subsubsection{{\sl ADMM $\subseteq$ projective dynamics: fast simulation of general constitutive models}\cite{ADMM_Proj}}
