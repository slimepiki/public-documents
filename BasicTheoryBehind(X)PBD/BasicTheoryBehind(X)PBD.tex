...
If the force $F$ is constant, the quantity $W$ measured the below equation defined by the force $F$ and moved distance $x$,
...
\begin{eqnarray}
	\frac{dC(\boldsymbol{x})}{dt} &=& \boldsymbol{x} \cdot \frac \boldsymbol{v}\\
	\frac{d^2C(\boldsymbol{x})}{dt}^2 &=&  \boldsymbol{a} \cdot \boldsymbol{x} + \boldsymbol{v} \cdot \boldsymbol{v}
\end{eqnarray}
...
\begin{eqnarray}
	...
	\frac{d^2C(\boldsymbol{x})}{dt}^2 &=& 0.
\end{eqnarray}

Thus, $\boldsymbol{x}$ is regal if $C(\boldsymbol{x}) = 0$, and $\boldsymbol{v}$ is regal if $\frac{dC(\boldsymbol{x})}{dt} = 0$, futhermore, $\boldsymbol{a}$ is regal if $\frac{d^2C(\boldsymbol{x})}{dt^2} = 0$

In particular, the last relation is heavily used in force-based constrained dynamics when solving the system.
